...
import ewatercycle_wflowjl.forcing.makkink as makkink
...
def create_provenance_record():
    ...
    record = {
        "caption":
        "Forcings for the Wflow.jl hydrological model.",
        "domains": ["global"],
        "authors": [
            "kalverla_peter",
            "camphuijsen_jaro",
...
def save(cubes, dataset, provenance, cfg):
    ...
    iris.save(cubes, output_file, fill_value=1.e20, zlib=True)
...
def main(cfg):
    ...
    for dataset, metadata in group_metadata(input_metadata, "dataset").items():
        ...
        if dataset == "ERA5":
            logger.info("Potential evapotransporation not available, deriving with de Bruin equation.")
            ...
            logger.info("Potential evapotransporation not available, deriving with Makkink equation.")
            rsds_dem = regrid(all_vars["rsds"], dem, scheme)
