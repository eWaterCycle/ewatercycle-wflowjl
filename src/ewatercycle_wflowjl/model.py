...
import toml
from ewatercycle.util import get_time, geographical_distances
...
def get_value_as_xarray(model: "eWaterCycleModel", name: str) -> xr.DataArray:
    ...
    da = da.expand_dims(dim = {"time": [model.time_as_datetime],})
    return da


def coords_to_indices(
    model: eWaterCycleModel,
    name: str,
    lat: Iterable[float],
    lon: Iterable[float]
    ) -> np.ndarray:
...
class WflowJlMixins(eWaterCycleModel):
    ...
    def _make_cfg_file(self, **kwargs) -> Path:
        ...
        # Output paths
        self._config["state"]["path_output"] = str(self._cfg_dir / self._config["state"]["path_output"])
        self._config["output"]["path"] = str(self._cfg_dir / self._config["output"]["path"])
        self._config["csv"]["path"] = str(self._cfg_dir / self._config["csv"]["path"])
        self._config["netcdf"]["path"] = str(self._cfg_dir / self._config["netcdf"]["path"])

        # TODO: add the following to toml (check which ones are required when).
